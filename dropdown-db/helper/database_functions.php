...
function generateInputField($spalten){
   $inputField = "";
    foreach ($spalten as $spalte ) {
        $inputField.= '<div class="form-group m-3">';
        $spaltenname = $spalte['Field'];
        $inputField.=  '<label>'. $spaltenname . '</label>';
        $inputField.='<input type="text" class="form-control" placeholder="'. $spaltenname . '">';
        $inputField.='</div>';
    }    
    return $inputField;
}

function tablesDropdown($tables){
   $html=' <form class="card p-5 border-0 shadow" method="post">
  <div class="form-group">
    <label for="tabellen">Tabellen</label>';
   $html .= createDropdown('tabellen',$tables);
  $html.='</div>
